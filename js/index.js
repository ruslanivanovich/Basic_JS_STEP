...
document.addEventListener('DOMContentLoaded',()=>{
	...
	function showCards(trainers){
		...
		trainers.forEach(trainerCharacter =>{
		const trainerTemplateCards = document.getElementById('trainer-card')
		const cardsTemplateContent = trainerTemplateCards.content.cloneNode(true)
		const trainerImage = cardsTemplateContent.querySelector('.trainer__img')
		const trainerInfo = cardsTemplateContent.querySelector('.trainer__name')
		const moreInfo = cardsTemplateContent.querySelector('.trainer__show-more')
		trainerImage.src = trainerCharacter.photo
		trainerInfo.textContent = trainerCharacter["last name"] + ' ' + trainerCharacter["first name"]
		trainerContainer.append(cardsTemplateContent)

		moreInfo.addEventListener('click', () => {
      		showModal(trainerCharacter)
    	})
		trainerContainer.append(cardsTemplateContent);
		})
	}
	function showModal(trainersModal){
				const modalTemplate = document.getElementById('modal-template')
				const templateContent = modalTemplate.content.cloneNode(true)
				const templateContainer = templateContent.querySelector('.modal')
				const trainerTemplateName = templateContent.querySelector('.modal__name')
				const trainerTemplateCategory = templateContent.querySelector('.modal__point--category')
				const trainerTemplateExperience = templateContent.querySelector('.modal__point--experience')
				const trainerTemplateSpecialization = templateContent.querySelector('.modal__point--specialization')
				const trainerTemplateText = templateContent.querySelector('.modal__text')
				const trainerTemplateImage = templateContent.querySelector('.modal__img')
				const modalCloseButton = templateContent.querySelector('.modal__close')
				trainerTemplateName.textContent = trainersModal["last name"] + ' ' + trainersModal["first name"]
				trainerTemplateCategory.textContent = 'Категорія: ' + ' ' + trainersModal.category
				trainerTemplateExperience.textContent = 'Досвід: ' + ' ' + trainersModal.experience
				trainerTemplateSpecialization.textContent = 'Напрям тренера: ' + ' ' + trainersModal.specialization
				trainerTemplateText.textContent  = trainersModal.description
				trainerTemplateImage.src = trainersModal.photo
				...
				trainerContainer.append(templateContent)
	}
	
	showCards(DATA)

	const sortTrainers = document.querySelector('.sorting')
	sortTrainers.removeAttribute('hidden')
	const filterBar = document.querySelector('.sidebar')
	...
	const sortButton = document.querySelectorAll('.sorting__btn')
		sortButton[0].addEventListener('click',()=>{
				sortButton.forEach(button =>{
					...
					sortButton[0].classList.add('sorting__btn--active')
				})	
			showCards(originalData)

		})
		sortButton[1].addEventListener('click',()=>{
				sortButton.forEach(button =>{
					button.classList.remove('sorting__btn--active')
					sortButton[1].classList.add('sorting__btn--active')
				})	
				const sortedBySurname = [...DATA].sort((a,b)=>{
				const surnameFirstLetter = a["last name"].split(' ')[0]
				const surnameSecondLetter = b["last name"].split(' ')[0]
				return surnameFirstLetter.localeCompare(surnameSecondLetter, 'uk')
			})
			showCards(sortedBySurname)
			 
		})
		sortButton[2].addEventListener('click',()=>{
				sortButton.forEach(button =>{
					button.classList.remove('sorting__btn--active')
					sortButton[2].classList.add('sorting__btn--active')
				})	
				const sortedByExperiance = [...DATA].sort((a,b)=>{
					return parseInt(b.experience) - parseInt(a.experience)
				})
				showCards(sortedByExperiance)
					
		})

		const showButton = document.querySelector('.filters__submit')
		function filterData(translated){
			showButton.addEventListener('click',(event)=>{
			event.preventDefault()
			const radioButDirection = document.querySelector('input[name="direction"]:checked').value
			const radioButCategory = document.querySelector('input[name="category"]:checked').value
			const filteredData = DATA.filter(trainer => {
				const translatedDirection = translated[radioButDirection];
				const translatedCategory = translated[radioButCategory];

				const matchDirection = radioButDirection === 'all' || trainer.specialization === translatedDirection;
				const matchCategory = radioButCategory === 'all' || trainer.category === translatedCategory;
